...
export default {
  ...
  methods: {
    addFood (target) { // 父组件用@cartAdd接收子组件传递的事件
      // console.log('调用父组件方法:' + target);
      this._drop(target);
    },
    _drop (target) {
      // 体验优化,异步执行下落动画
      this.$nextTick(() => {
        this.$refs.shopcart.drop(target); // 父组件调用子组件方法需用ref来调用
      });
    },
    _initScroll () {
    ...
    selectMenu (index, event) {
      // 非better-scroll派发的（浏览器派发的return掉）
