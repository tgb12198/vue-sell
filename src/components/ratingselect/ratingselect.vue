...
export default {
  ...
  data () {
    return {
      tmp_selectType: this.selectType,
      tmp_onlyContent: this.onlyContent
    };
  },
  computed: {
    positives () {
  ...
  methods: {
    select (type, event) {
      ...
      this.tmp_selectType = type;
      this.$emit('select', this.tmp_selectType);// 向父组件分发一个事件
    },
    toggleContent (event) {
      if (!event._constructed) {
        return;
      }
      this.tmp_onlyContent = !this.onlyContent;
      this.$emit('toggle', this.tmp_onlyContent);
    }
  }
};
</script>
